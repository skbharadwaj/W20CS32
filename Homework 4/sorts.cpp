...
int main()
{
      ...
      // destruction time.
    {
     vector<Student> auxStudents(students);
      ...
      for(int i = 0;i < auxStudents.size();i++) {
        Student* nextElement = &(auxStudents[i]);
        studentptrs.push_back(nextElement);
      }
    ...
    students.clear();
    for(int i = 0;i < auxStudents.size();i++) students.push_back(*studentptrs[i]);
